...
- "아키텍처 다이어그램", "시스템 다이어그램", "구조 시각화"
- "excalidraw 생성", "excalidraw 파일", "excalidraw 다이어그램"
- "코드베이스 시각화", "아키텍처 그려줘"
- "PNG/SVG로 내보내기", "다이어그램 export"

### 사용하지 않을 때

- 피그마 디자인 구현 → `figma-plan` 또는 `figma-reference-plan`
- 단순 플로우차트/마인드맵 → Mermaid 또는 일반 마크다운
- UI 컴포넌트 설계 → `frontend-design`
