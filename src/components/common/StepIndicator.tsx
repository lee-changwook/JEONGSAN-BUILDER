      ...
      {STEPS.map(({ step, label }, idx) => {
        ...
        return (
          <div key={step} className="flex items-center">
            ...
            <div
              className={`flex items-center gap-2 text-sm transition-colors duration-150 ${isActive ? 'font-semibold text-gray-900' : isDone ? 'font-medium text-blue-600' : 'font-medium text-gray-400'
                }`}
            >
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-[13px] font-semibold transition-all duration-150 ${isDone
